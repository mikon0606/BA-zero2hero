...
contract MKStaking is OwnableUpgradeable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    event StakeSuccess(address indexed staking_owner, uint256 stake_amount);

    event UnstakeSuccess(address indexed staking_owner, uint256 unstake_amount);

    event ClaimRewardSuccess(address indexed staking_owner, uint256 reward);
    ...
    function initialize(
        ...
        __Ownable_init();
        require(
            IERC20(_staking_token).totalSupply() > 0,
            "invalid staking token"
        );
        require(
            IERC20(_reward_token).totalSupply() > 0,
            "invalid reward token"
        );
        require(
            _start_claim_block_id >= _start_stake_block_id,
            "invalid block index config"
        );
    ...
    function stake(uint256 amount) external {
        ...
        _update_staking_database(
            _msgSender(),
            rec.staked.add(amount),
            total_reward
        );
    ...
    function get_total_reward(
        ...
        uint256 current_accumulate_reward = accumulate_unit_reward[
            block.number
        ];
        if (reward_update_block_id < block.number) {
            uint256 unit_reward_increment = unit_reward.mul(
                block.number.sub(reward_update_block_id)
            );
            current_accumulate_reward = accumulate_unit_reward[
                reward_update_block_id
            ].add(unit_reward_increment);
        }
        ...
        uint256 current_reward = (
            current_accumulate_reward.sub(pre_unit_reward)
        ).mul(rec.staked).div(TOKEN_DECIMAL);
    ...
    function _commit_user_reward(
        ...
        uint256 reward = (latest_unit_reward.sub(pre_unit_reward))
            .mul(rec.staked)
            .div(TOKEN_DECIMAL);
    ...
    function _update_unit_reward_state() internal {
        ...
        uint256 accumulate_reward_increment = unit_reward.mul(
            current_block_id.sub(reward_update_block_id)
        );
        accumulate_unit_reward[current_block_id] = accumulate_reward_increment
            .add(accumulate_unit_reward[reward_update_block_id]);

        reward_update_block_id = block.number;

        if (current_block_id <= start_stake_block_id) {
            accumulate_unit_reward[
                start_stake_block_id
            ] = accumulate_unit_reward[current_block_id];
    ...
    function _update_unit_reward() internal {
        if (total_staked_amount > 0) {
            unit_reward = share_per_block.mul(TOKEN_DECIMAL).div(
                total_staked_amount
            );
        }
    }
    ...
    function set_start_unstake_block_id(
        uint256 _start_unstake_block_id
    ) external onlyOwner {
        start_unstake_block_id = _start_unstake_block_id;
    }

    function set_start_claim_block_id(
        uint256 _start_claim_block_id
    ) external onlyOwner {
        start_claim_block_id = _start_claim_block_id;
    }
